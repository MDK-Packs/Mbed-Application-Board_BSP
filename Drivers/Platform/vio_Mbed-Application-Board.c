/******************************************************************************
 * @file     vio_Mbed-Application Board.c
 * @brief    Virtual I/O implementation for board Mbed Application Board
 * @version  V1.0.0
 * @date     12. July 2021
 ******************************************************************************/
/*
...
/*! \page vio_Mbed-Application Board Physical I/O Mapping

The table below lists the physical I/O mapping of this CMSIS-Driver VIO implementation.

Virtual Resource  | Variable       | Physical Resource on Mbed Application Board    |
:-----------------|:---------------|:-----------------------------------------------|
vioLED4           | vioSignalOut.4 | P2_3/P23: Red                                  |
vioLED5           | vioSignalOut.5 | P2_2/P24: Green                                |
vioLED6           | vioSignalOut.6 | P2_1/P25: Blue                                 |
*/
...
void vioSetSignal(uint32_t mask, uint32_t signal)
{
    ...
    if ((mask & vioLED4) != 0U)
    {
        if ((signal & vioLED4) != 0U)
        {
            GPIO_PinWrite(2, 3, 1U);
        }
        else
        {
            GPIO_PinWrite(2, 3, 0U);
    ...
    if ((mask & vioLED5) != 0U)
    {
        if ((signal & vioLED5) != 0U)
        {
            GPIO_PinWrite(2, 2, 1U);
        }
        else
        {
            GPIO_PinWrite(2, 2, 0U);
    ...
    if ((mask & vioLED6) != 0U)
    {
        if ((signal & vioLED6) != 0U)
        {
            GPIO_PinWrite(2, 1, 1U);
        }
        else
        {
            GPIO_PinWrite(2, 1, 0U);
